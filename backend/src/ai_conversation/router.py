...
async def get_company_charity_info(
    ...
    # Очистка названия компании от организационно-правовой формы для более гибкого поиска
    clean_company_name = re.sub(r'^(ТОО|АО|ИП|A\.O\.|TOO|LLP|JSC|)\s*|"|«|»', '', company_name, flags=re.IGNORECASE).strip()
    print(f"   -> Очищенное название для поиска: '{clean_company_name}'")

    # Более широкий и гибкий набор ключевых слов для поиска благотворительности
    charity_keywords_ru = [
    ...
    if request.additional_context and request.additional_context.strip():
        ...
        # Улучшенный запрос с контекстом - ТОЛЬКО для Казахстана
        ru_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_ru[:6]])
        en_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_en[:4]])
        search_queries = [
            f'"{clean_company_name}" AND ("{context}" OR {ru_keywords} OR {en_keywords}) AND ("Казахстан" OR "Kazakhstan" OR site:kz)'
        ]
        print(f"📝 [CHARITY_RESEARCH] Создан 1 улучшенный запрос с контекстом (ограничен Казахстаном)")
    else:
        # Два улучшенных, но целенаправленных запроса - ТОЛЬКО для Казахстана
        ru_main_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_ru[:5]])
        en_main_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_en[:3]])
        search_queries = [
            # Запрос 1: Основные термины благотворительности (русский + английский) + географическое ограничение
            f'"{clean_company_name}" AND ({ru_main_keywords} OR {en_main_keywords}) AND ("Казахстан" OR "Kazakhstan" OR site:kz)',
            
            # Запрос 2: Конкретные благотворительные действия + географическое ограничение
            f'"{clean_company_name}" AND ("помощь фонду" OR "поддержал проект" OR "социальная помощь" OR "charity program" OR "CSR") AND ("Казахстан" OR "Kazakhstan" OR site:kz)'
        ]
        print(f"📝 [CHARITY_RESEARCH] Созданы 2 улучшенных целенаправленных запроса (ограничены Казахстаном)")
            ...
            try:
                ...
                if 'items' in search_data:
                    for item in search_data['items']:
                        ...
                        # 🎯 УЛУЧШЕННАЯ ФИЛЬТРАЦИЯ: Используем очищенное название компании
                        company_name_variants = [
                            company_name.lower(),
                            clean_company_name.lower(),  # очищенное название
                            company_name.lower().replace('"', ''),  # без кавычек
                            clean_company_name.lower().replace('"', ''),  # очищенное без кавычек
                        ]
                        ...
                        # 🔍 СТРОГИЕ КРИТЕРИИ ДЛЯ КАЗАХСТАНА: результат принимается только если:
                        # 1. Есть название компании 2. Есть ключевые слова благотворительности 
                        # 3. Нет исключающих слов 4. Релевантен Казахстану и НЕ из других стран
                        ...
                        if is_acceptable:
                            ...
                            found_relevant += 1
                            print(f"✅ [CHARITY_RESEARCH] Казахстанский фильтр ПРОЙДЕН: {item.get('title', '')[:50]}...")
                        else:
                            ...
                            print(f"🚫 [CHARITY_RESEARCH] Казахстанский фильтр НЕ ПРОЙДЕН ({', '.join(reasons)}): {item.get('title', '')[:50]}...")
    ...
    if not all_search_results:
        final_summary_for_response = (
            ...
            f"Поиск проводился исключительно по казахстанским ресурсам (.kz домены) и материалам, "
            f"касающимся деятельности в Казахстане.\n\n"
            f"Возможные причины:\n"
            f"• Компания не ведет публичную благотворительную деятельность в Казахстане\n"
            f"• Социальные проекты не освещаются в казахстанских интернет-ресурсах\n"
            f"• Благотворительность ведется через дочерние структуры\n\n"
            f"Рекомендация: обратитесь напрямую к казахстанским представителям компании для уточнения возможностей спонсорской поддержки."
        )
    else:
        ...
        if direct_evidence_count > 0:
            ...
            final_summary_for_response = (
                f"НАЙДЕНЫ ПРЯМЫЕ ДОКАЗАТЕЛЬСТВА благотворительной деятельности компании '{company_name}' В КАЗАХСТАНЕ.\n\n"
                f"Обнаружено {direct_evidence_count} казахстанских источников с конкретными фактами благотворительности "
                f"(из {len(all_search_results)} проверенных материалов из .kz ресурсов).\n\n"
                f"Подтвержденная активность в Казахстане: {activities_text}\n"
                f"Области деятельности: {areas_text}\n\n"
                f"Компания ДЕЙСТВИТЕЛЬНО занимается благотворительностью в Казахстане. "
                f"Рекомендуется изучить приложенные казахстанские источники и обратиться в местный отдел КСО компании."
            )
        else:
            final_summary_for_response = (
                f"Найдено {len(all_search_results)} упоминаний компании '{company_name}' в КАЗАХСТАНСКИХ источниках в контексте благотворительности, "
                f"НО НЕТ ПРЯМЫХ ДОКАЗАТЕЛЬСТВ конкретных благотворительных действий в Казахстане.\n\n"
                f"Обнаруженная информация из .kz ресурсов носит общий характер (упоминания в списках, новости без подробностей, "
                f"декларации о социальной ответственности без конкретных проектов).\n\n"
                f"Рекомендация: требуется дополнительная проверка через официальные казахстанские каналы компании."
            )
    ...
    if not all_search_results:
        print(f"🇰🇿 [CHARITY_RESEARCH] Завершено исследование компании '{company_name}' (очищено: '{clean_company_name}') в КАЗАХСТАНЕ: 0 релевантных результатов из {total_queries} улучшенных запросов")
        print(f"📊 [CHARITY_RESEARCH] Экономия API квот: использовано {total_queries} запросов вместо 8-12 (поиск только по Казахстану)")
    ...
    print(f"✅🇰🇿 [CHARITY_RESEARCH] Исследование завершено для '{company_name}' (очищено: '{clean_company_name}') в КАЗАХСТАНЕ: найдено {total_results} релевантных результатов из {total_queries} улучшенных запросов")
    print(f"📊 [CHARITY_RESEARCH] Экономия API квот: использовано {total_queries} запросов вместо 8-12 (поиск только по Казахстану, экономия ~{8-total_queries} запросов)")
