...
async def get_company_charity_info(
    ...
    # Исключающие слова для поиска (более осторожно, чтобы не отсечь релевантные новости)
    exclude_search_terms = [
        "государственные закупки", "тендер", "вакансия", "работа", 
        "услуги", "прайс", "каталог", "оптовая торговля",
        "помощь по услугам", "судебные дела", "труда", # "труда" чтобы отсечь "фонд оплаты труда"
        "пенсионный", "накопительный", # Чтобы отсечь пенсионные фонды
        "основные фонды" # Точно исключаем этот термин
    ]
    ...
    # Исключающие ключевые слова для фильтрации (менее агрессивно, чтобы не отсечь отчеты КСО)
    exclude_keywords = [
        ...
        'договор', 'контрагент', 'техническая поддержка',
        # Оставляем только очень сильные индикаторы нерелевантности, не общие слова из новостей/отчетов
        'advertisement', 'vacancy', 'job', 'sale', 'services'
    ]
                ...
                if 'items' in search_data:
                    for item in search_data['items']:
                        ...
                        strong_positive_phrases = [
                            ...
                            "перечислил в фонд", # ОЧЕНЬ ВАЖНО для Kaspi
                            "предоставил платформу для пожертвований", # ОЧЕНЬ ВАЖНО для Kaspi.kz
                            "крупный донор", "поддержал фонд", "организовал сбор средств",
                            "внес вклад в фонд", "партнер благотворительной организации",
                            "перечислил", "тенге", "миллион", "млн", "млрд", # Финансовые показатели
                            "благотворительная платформа", "платформа для сбора", "сбор средств",
                            "социальные инвестиции", "корпоративная социальная ответственность",
                            "КСО программа", "CSR initiative", "charitable foundation"
                        ]
                        ...
                        # Улучшенная проверка контекста "фонда": более гибкая логика
                        if "фонд" in full_text and "благотворительный" not in full_text:
                            # Проверяем на явно неблаготворительные контексты
                            if any(term in full_text for term in ["пенсионный", "основные", "оплаты труда", "государственный"]):
                                negative_score += 3  # Умеренно снижаем релевантность
                            # Но если есть другие позитивные индикаторы, не наказываем слишком сильно
                            elif positive_score > 0:
                                negative_score += 1  # Минимальное снижение
                        ...
                        # 1. Есть название компании 2. Есть релевантность благотворительности
                        if has_company_name and is_charity_relevant:
                            ...
                            # Детальное логирование причин отклонения с полной информацией
                            reasons = []
                            ...
                            debug_info = f"Title: '{item.get('title', '')[:80]}...' | Snippet: '{item.get('snippet', '')[:100]}...'"
                            print(f"🚫 [CHARITY_RESEARCH] Строгий фильтр НЕ ПРОЙДЕН ({', '.join(reasons)})")
                            print(f"🔍 [CHARITY_RESEARCH] Отладочная информация: {debug_info}")
    ...
    if not all_search_results:
        ...
        direct_action_keywords = [
            ...
            # Ключевые слова для платформ и доноров (ОЧЕНЬ ВАЖНО для Kaspi)
            'перечислил в фонд', 'предоставил платформу', 'собрал средства для', 
            'донор фонда', 'организовал сбор средств', 'платформа для пожертвований',
            ...
            # Специфичные для финансовых компаний и платформ
            'перечислил', 'внес вклад', 'партнер благотворительной', 'социальные инвестиции',
            'корпоративная социальная ответственность', 'КСО', 'CSR', 'charitable foundation',
            'благотворительная платформа', 'платформа для сбора', 'сбор средств'
        ]
        ...
        for result in all_search_results:
            ...
            is_platform = any(kw in text for kw in ['предоставил платформу', 'платформа для пожертвований', 'благотворительная платформа'])
            is_major_donor = any(kw in text for kw in ['перечислил в фонд', 'донор фонда', 'собрал средства для', 'организовал сбор средств'])
            is_financial_company = any(kw in text for kw in ['перечислил', 'внес вклад', 'социальные инвестиции', 'КСО', 'CSR'])
            
            if has_direct_action or is_platform or is_major_donor or is_financial_company:
                ...
                if is_financial_company:
                    specific_activities.append('финансовая поддержка/социальные инвестиции')
