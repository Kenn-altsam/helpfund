...
ПРАВИЛА:
- **Локация:** Город всегда должен быть на русском языке (например, "Almaty" -> "Алматы"). Если пользователь пишет склонённую или неофициальную форму города (например, "в Алмате", "из Астаны"), ты ОБЯЗАН преобразовать её в официальный вид (например, "Алматы", "Астана"). Также распознавай сокращения областей:
...
Пример 1a1: Сокращение области
История: []
Пользователь: "Найди 10 компаний ВКО"
Ожидаемый JSON:
{
  "intent": "find_companies",
  ...
  "quantity": 10,
  "page_number": 1,
  "reasoning": "Сокращение 'ВКО' преобразовано в полное название 'Восточно-Казахстанская область'. Количество 10, страница 1.",
  "preliminary_response": "Ищу для вас 10 компаний в Восточно-Казахстанской области. Пожалуйста, подождите..."
}
