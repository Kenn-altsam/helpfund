...
async def get_company_charity_info(
    ...
    # Расширенные ключевые слова для определения релевантности благотворительности
    charity_keywords = [
        ...
        'initiative', 'program', 'social responsibility', 'волонтерство', 'экология',
        'культура', 'спорт', 'медицина', 'университет', 'школа', 'детский сад',
        'инвалиды', 'малообеспеченные', 'пенсионеры', 'ветераны', 'корпоративная социальная ответственность'
    ]
            ...
            try:
                ...
                if 'items' in search_data:
                    for item in search_data['items']:
                        ...
                        # 🔍 ГИБКИЕ КРИТЕРИИ: результат принимается если:
                        # 1. Есть название компании И 2. Есть ключевые слова благотворительности
                        # Исключающие слова не блокируют полностью, а только снижают приоритет
    ...
    # 🎯 ГИБКАЯ ГЕНЕРАЦИЯ СВОДКИ: анализируем все найденные материалы
    if not all_search_results:
        final_summary_for_response = (
            ...
            f"• Информация может быть на казахском языке или в локальных источниках\n"
            f"• Компания может использовать другие названия для благотворительных проектов\n\n"
            f"Рекомендация: обратитесь напрямую к представителям компании для уточнения возможностей спонсорской поддержки. "
            f"Многие казахстанские компании ведут благотворительную деятельность, но не всегда публикуют информацию об этом в открытых источниках."
        )
    else:
        ...
        # Расширенные ключевые слова для определения благотворительных действий
        direct_action_keywords = [
            ...
            'поддерживает', 'помогает', 'участвует', 'организует', 'проводит',
            'supports', 'helps', 'participates', 'organizes', 'conducts',
            'реализует', 'осуществляет', 'ведет', 'развивает', 'создает'
        ]
        
        for result in all_search_results:
            text = (result.title + " " + result.snippet).lower()
            
            # Проверяем наличие прямых действий или упоминаний благотворительности
            has_direct_action = any(action in text for action in direct_action_keywords)
            has_charity_mention = any(word in text for word in ['благотворительность', 'charity', 'благотворительный', 'charitable'])
            has_social_responsibility = any(word in text for word in ['социальная ответственность', 'КСО', 'CSR', 'корпоративная социальная ответственность'])
            
            if has_direct_action or has_charity_mention or has_social_responsibility:
            ...
            # Анализ областей деятельности (при наличии любых признаков благотворительности)
            if has_direct_action or has_charity_mention or has_social_responsibility:
        ...
        if direct_evidence_count > 0:
            ...
            final_summary_for_response = (
                f"НАЙДЕНА ИНФОРМАЦИЯ о благотворительной деятельности компании '{company_name}'.\n\n"
                f"Обнаружено {direct_evidence_count} источников с упоминаниями благотворительности "
                f"(из {len(all_search_results)} проверенных материалов).\n\n"
                f"Выявленная активность: {activities_text}\n"
                f"Области деятельности: {areas_text}\n\n"
                f"Компания проявляет активность в сфере социальной ответственности. "
                f"Рекомендуется изучить приложенные источники и обратиться в отдел КСО компании для получения более подробной информации."
            )
        else:
            final_summary_for_response = (
                f"Найдено {len(all_search_results)} упоминаний компании '{company_name}' в контексте благотворительности, "
                f"НО ИНФОРМАЦИЯ НОСИТ ОБЩИЙ ХАРАКТЕР.\n\n"
                f"Обнаруженная информация может включать:\n"
                f"• Упоминания в списках благотворительных организаций\n"
                f"• Новости без конкретных деталей проектов\n"
                f"• Общие заявления о социальной ответственности\n"
                f"• Косвенные упоминания в партнерских материалах\n\n"
                f"Рекомендация: обратитесь напрямую к представителям компании для уточнения возможностей спонсорской поддержки и получения информации о конкретных благотворительных проектах."
            )
    ...
    if not all_search_results:
        print(f"🔍 [CHARITY_RESEARCH] Завершено исследование компании '{company_name}': 0 релевантных результатов из {total_queries} расширенных запросов")
        print(f"📊 [CHARITY_RESEARCH] Использовано {total_queries} запросов с улучшенными критериями поиска")
    ...
    print(f"✅ [CHARITY_RESEARCH] Исследование завершено для '{company_name}': найдено {total_results} релевантных результатов из {total_queries} расширенных запросов")
    print(f"📊 [CHARITY_RESEARCH] Использовано {total_queries} запросов с улучшенными критериями поиска")
