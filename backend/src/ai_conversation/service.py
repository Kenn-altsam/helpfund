...
# <<< УЛУЧШЕННЫЙ ШАБЛОН ПРОМПТА ДЛЯ АНАЛИЗА БЛАГОТВОРИТЕЛЬНОСТИ >>>
CHARITY_SUMMARY_PROMPT_TEMPLATE = """
Ты - эксперт по анализу корпоративной социальной ответственности и благотворительной деятельности компаний в Казахстане. Проанализируй найденную информацию о компании "{company_name}" и составь детальную сводку.

НАЙДЕННЫЕ МАТЕРИАЛЫ:
{search_results_text}

ЗАДАЧА АНАЛИЗА:
1. **Всесторонний анализ благотворительной деятельности:**
   - Прямые пожертвования и финансирование
   - Социальные проекты и инициативы
   - Корпоративная социальная ответственность (КСО)
   - Спонсорство мероприятий и программ
...
2. **Оценка достоверности информации:**
   - Официальные источники (сайт компании, пресс-релизы)
   - Новостные публикации
   - Отчеты о социальной ответственности
   - Упоминания в партнерских организациях

3. **Структурированный вывод:**
   - Типы выявленной благотворительной деятельности
   - Конкретные проекты и инициативы
   - Временные рамки (если указаны)
   - Суммы или масштаб деятельности (если указаны)
   - Партнеры и получатели помощи
   - Оценка достоверности информации

ФОРМАТ ОТВЕТА:
Используй следующий структурированный формат:

**НАЙДЕННАЯ ИНФОРМАЦИЯ:**
[Краткое описание найденных материалов и их источников]

**ТИПЫ БЛАГОТВОРИТЕЛЬНОЙ ДЕЯТЕЛЬНОСТИ:**
[Список выявленных направлений: пожертвования, социальные проекты, спонсорство и т.д.]

**КОНКРЕТНЫЕ ПРОЕКТЫ И ИНИЦИАТИВЫ:**
[Детальное описание найденных проектов с датами, суммами, получателями]

**ОЦЕНКА ДОСТОВЕРНОСТИ:**
[Оценка надежности источников и полноты информации]

**РЕКОМЕНДАЦИИ:**
[Советы по дальнейшему исследованию или обращению к компании]

ВАЖНЫЕ ПРИНЦИПЫ:
- Будь максимально объективным и честным
- Не выдумывай информацию, которой нет в источниках
- Учитывай как прямые, так и косвенные признаки благотворительности
- Учитывай специфику казахстанского бизнеса и культуры
- Если информация общая или неконкретная - честно об этом скажи
- Давай практические рекомендации для дальнейших действий

Если информации недостаточно, структурированно объясни, что именно не найдено и как можно получить дополнительную информацию.
"""
...
Пожалуйста, выберите одну из доступных областей или укажите конкретный город."""
    ...
    async def _research_charity_online(self, company_name: str) -> str:
        ...
        # УЛУЧШЕНИЕ 2: Расширенные ключевые слова для всестороннего поиска
        core_charity_terms = [
            "благотворительность", "пожертвования", "спонсорство", 
            "социальная ответственность", "помощь", "поддержка",
            "КСО", "CSR", "корпоративная социальная ответственность"
        ]
        
        specific_charity_actions = [
            "детский дом", "фонд", "образование", "здравоохранение",
            "помощь малообеспеченным", "социальный проект", "волонтерство",
            "экология", "культура", "спорт", "медицина", "университет"
        ]

        # УЛУЧШЕНИЕ 3: Три стратегических запроса для максимального покрытия
        # Запрос 1: Основные благотворительные термины
        query_1 = f'"{clean_company_name}" Казахстан ({" OR ".join(core_charity_terms)})'
        
        # Запрос 2: Конкретные благотворительные действия
        query_2 = f'"{clean_company_name}" ({" OR ".join(specific_charity_actions)})'
        
        # Запрос 3: Поиск по социальным проектам и инициативам
        query_3 = f'"{clean_company_name}" ("социальный проект" OR "социальная инициатива" OR "благотворительный фонд" OR "фонд помощи")'

        queries_to_execute = [query_1, query_2, query_3]
    ...
    def _is_charity_relevant(self, title: str, snippet: str) -> bool:
        ...
        Использует более гибкие критерии для лучшего покрытия.
        """
        combined_text = f"{title} {snippet}".lower()
        
        # Расширенные позитивные индикаторы благотворительности
        positive_indicators = [
            "благотворительность", "пожертвование", "спонсорство", "помощь", 
            "поддержка", "фонд", "социальная ответственность", "CSR", "КСО",
            "детский дом", "больница", "образование", "стипендия",
            "волонтер", "донор", "меценат", "гранты", "социальный проект",
            "корпоративная социальная ответственность", "экология", "культура",
            "спорт", "медицина", "университет", "школа", "детский сад",
            "инвалиды", "малообеспеченные", "пенсионеры", "ветераны"
        ]
        ...
        negative_indicators = [
            ...
            "вакансия", "работа", "резюме", "сотрудник", "зарплата",
            "отзыв", "жалоба", "скандал", "коррупция", "штраф"
        ]
        
        # Подсчитываем релевантность с более гибкими критериями
        ...
        # Более гибкие критерии: результат релевантен, если есть позитивные индикаторы
        # и негативных не больше чем позитивных + 1 (допускаем небольшой шум)
        is_relevant = positive_score > 0 and negative_score <= (positive_score + 1)
        
        if not is_relevant:
            print(f"   -> Filtered out non-relevant result: {title[:50]}... (pos: {positive_score}, neg: {negative_score})")
        else:
            print(f"   -> Accepted relevant result: {title[:50]}... (pos: {positive_score}, neg: {negative_score})")
