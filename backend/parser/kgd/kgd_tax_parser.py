        ...
        try:
            ...
            for row in rows:
                ...
                if len(cells) >= 7:  # Should have 7 columns as per the HTML structure
                    ...
                    # First row should be "Налоговые поступления"
                    if "налоговые поступления" in cell_texts[0].lower():
                        # Map the tax amounts to years
                        if len(cell_texts) >= 7:
                            # Clean up the tax amounts (remove non-numeric characters except digits and commas/dots)
