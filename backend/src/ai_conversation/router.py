...
async def get_company_charity_info(
    ...
    if request.additional_context and request.additional_context.strip():
        ...
        # Запрос с дополнительным контекстом
        search_queries = [
            f'"{company_name}" "{context}"',
            f'"{company_name}" благотворительность',
            f'"{company_name}" социальная ответственность'
        ]
        print(f"📝 [CHARITY_RESEARCH] Созданы 3 запроса с контекстом")
    else:
        # Три простых и эффективных запроса
        search_queries = [
            f'"{company_name}" благотворительность',
            f'"{company_name}" социальная ответственность', 
            f'"{company_name}" пожертвования фонд'
        ]
        print(f"📝 [CHARITY_RESEARCH] Созданы 3 простых запроса")
