...
async def get_company_charity_info(
    ...
    if request.additional_context and request.additional_context.strip():
        ...
        # Улучшенный запрос с контекстом - используем очищенное название компании
        ru_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_ru[:6]])
        en_keywords = " OR ".join([f'"{kw}"' for kw in charity_keywords_en[:4]])
        search_queries = [
            f'"{clean_company_name}" AND ("{context}" OR {ru_keywords} OR {en_keywords})'
        ]
        print(f"📝 [CHARITY_RESEARCH] Создан 1 улучшенный запрос с контекстом")
    else:
        # Два улучшенных, но целенаправленных запроса
        ...
        search_queries = [
            # Запрос 1: Основные термины благотворительности (русский + английский)
            f'"{clean_company_name}" AND ({ru_main_keywords} OR {en_main_keywords})',
            
            # Запрос 2: Конкретные благотворительные действия  
            f'"{clean_company_name}" AND ("помощь фонду" OR "поддержал проект" OR "социальная помощь" OR "charity program" OR "CSR")'
        ]
        print(f"📝 [CHARITY_RESEARCH] Созданы 2 улучшенных целенаправленных запроса")
    ...
    if not all_search_results:
        print(f"🔍 [CHARITY_RESEARCH] Завершено исследование компании '{company_name}' (очищено: '{clean_company_name}'): 0 релевантных результатов из {total_queries} улучшенных запросов")
    ...
    print(f"✅ [CHARITY_RESEARCH] Исследование завершено для '{company_name}' (очищено: '{clean_company_name}'): найдено {total_results} релевантных результатов из {total_queries} улучшенных запросов")
