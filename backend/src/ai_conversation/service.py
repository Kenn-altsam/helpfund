...
ПРАВИЛА:
- **Локация:** Город или область всегда должны быть на русском языке (например, "Almaty" -> "Алматы"). Если пользователь пишет склонённую или неофициальную форму города или области (например, "в Алмате", "из Астаны", "в Улытауской области"), ты ОБЯЗАН преобразовать её в официальный вид (например, "Алматы", "Астана", "Улытауская область"). Для областей всегда используй полную каноническую форму с "область" заканчивающуюся на "-ая область" (например, "Улытауская область", "Алматинская область", "Актюбинская область"). Обрабатывай сокращенные формы областей: "области Жетысу" -> "Жетисуская область", "области Актобе" -> "Актюбинская область", "области Атырау" -> "Атырауская область". Если город или область не указаны ни в текущем сообщении, ни в истории, `location` должен быть `null`.
...
Ожидаемый JSON:
{
  ...
  "quantity": 10,
  "page_number": 1,
  "reasoning": "Указана сокращенная форма области 'области Жетысу', преобразовал в каноническую форму 'Жетисуская область'. Ключевые слова: крупных. Количество 10, страница 1.",
  "preliminary_response": "Ищу 10 крупных компаний в Жетисуской области. Пожалуйста, подождите..."
}

Пример 1f: Поиск по сокращенной форме области Актобе
История: []
...
Ожидаемый JSON:
{
  ...
  "quantity": 10,
  "page_number": 1,
  "reasoning": "Указана сокращенная форма области 'области Актобе', преобразовал в каноническую форму 'Актюбинская область'. Количество по умолчанию 10, страница 1.",
  "preliminary_response": "Ищу компании в Актюбинской области. Пожалуйста, подождите..."
}

Пример 1g: Поиск по сокращенной форме области Атырау
История: []
Пользователь: "Найди нефтяные компании области Атырау"
Ожидаемый JSON:
{
  "intent": "find_companies",
  "location": "Атырауская область",
  "activity_keywords": ["нефтяные"],
  "quantity": 10,
  "page_number": 1,
  "reasoning": "Указана сокращенная форма области 'области Атырау', преобразовал в каноническую форму 'Атырауская область'. Ключевые слова: нефтяные. Количество по умолчанию 10, страница 1.",
  "preliminary_response": "Ищу нефтяные компании в Атырауской области. Пожалуйста, подождите..."
}
    ...
    async def handle_conversation_turn(self, user_input: str, history: List[Dict[str, str]], db: Session, conversation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if intent == "find_companies" and location:
            ...
            if db_companies:
                ...
                # Проверяем, является ли локация областью
                if "область" in location:
                    final_message = f"Я искал компании в {location}, но в базе данных нет информации для этой области. Возможно, это новая область или данные еще не добавлены. Попробуйте поискать в других областях или городах."
                else:
