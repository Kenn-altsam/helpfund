...
async def get_company_charity_info(
    ...
    # === УЛУЧШЕНИЕ 1: Очистка названия компании от организационно-правовой формы ===
    # Убираем "ТОО", "АО", кавычки и т.д., чтобы искать только суть названия.
    # Это решает проблему несовпадения "ТОО МЕТРО" и "«МЕТРО»".
    ...
    # === УЛУЧШЕНИЕ 2: Более широкий и гибкий список ключевых слов для ПРОШЛЫХ действий ===
    # Фокус на том, что компания УЖЕ ДЕЛАЛА в прошлом
    past_charity_keywords_ru = [
        "выделил", "профинансировал", "пожертвовал", "передал", "спонсировал", "поддержал",
        "помог", "построил", "организовал", "провел", "реализовал", "запустил",
        "благотворительная деятельность", "социальные проекты", "корпоративная социальная ответственность"
    ]
    
    past_charity_keywords_en = [
        "donated", "funded", "sponsored", "supported", "contributed", "allocated",
        "charity", "CSR", "corporate social responsibility", "philanthropy", "social impact"
    ]

    # 🚀 ОПТИМИЗИРОВАННЫЕ ЗАПРОСЫ: ВСЕГО 1-2 ЗАПРОСА ВМЕСТО 8+
    # Используем очищенное название + ключевые слова прошлых действий
    ...
    if request.additional_context and request.additional_context.strip():
        ...
        # Улучшенный запрос с контекстом - фокус на прошлых действиях
        search_queries = [
            f'("{clean_company_name}" OR "{company_name}") AND ("{context}" OR "выделил" OR "профинансировал" OR "пожертвовал" OR "спонсировал" OR "donated" OR "funded" OR "sponsored") AND ("благотворительность" OR "charity" OR "КСО" OR "CSR")'
        ]
        print(f"📝 [CHARITY_RESEARCH] Создан 1 улучшенный запрос с контекстом (фокус на прошлых действиях)")
    else:
        # Два улучшенных запроса с фокусом на конкретные прошлые действия
        search_queries = [
            # Запрос 1: Русские термины прошлых действий + очищенное название
            f'("{clean_company_name}" OR "{company_name}") AND ("выделил" OR "профинансировал" OR "пожертвовал" OR "спонсировал" OR "поддержал" OR "помог") AND ("благотворительность" OR "фонд" OR "социальные проекты" OR "КСО")',
            
            # Запрос 2: Английские термины + широкий поиск по активности
            f'("{clean_company_name}" OR "{company_name}") AND ("donated" OR "funded" OR "sponsored" OR "contributed") AND ("charity" OR "foundation" OR "CSR" OR "social responsibility")'
        ]
        print(f"📝 [CHARITY_RESEARCH] Созданы 2 улучшенных запроса (русский + английский, фокус на прошлых действиях)")

    all_search_results: List[GoogleSearchResult] = []
    
    # === УЛУЧШЕНИЕ 3: Обновленные ключевые слова для поиска ПРОШЛЫХ благотворительных действий ===
    charity_keywords = [
        # Русские ключевые слова - прошлые действия
        'выделил', 'профинансировал', 'пожертвовал', 'передал', 'спонсировал', 'поддержал',
        'помог', 'построил', 'организовал', 'провел', 'реализовал', 'запустил',
        ...
        'детский дом', 'социальный проект', 'благотворительная акция', 'меценатство',
        # Английские ключевые слова - прошлые действия
        'donated', 'funded', 'sponsored', 'contributed', 'allocated', 'supported',
        'charity', 'charitable', 'foundation', 'donates', 'sponsors', 'supports',
        'initiative', 'program', 'social responsibility', 'philanthropy', 'CSR',
        'community support', 'social impact', 'corporate giving'
    ]
    
    # === УЛУЧШЕНИЕ 4: Расширенный список исключающих ключевых слов ===
    exclude_keywords = [
        # Работа и карьера
        'вакансия', 'работа', 'карьера', 'резюме', 'собеседование', 'трудоустройство',
        'vacancy', 'job', 'career', 'resume', 'interview', 'employment',
        # Коммерческая деятельность
        'продажа', 'купить', 'цена', 'стоимость', 'прайс', 'скидка', 'акция', 'распродажа',
        'sale', 'buy', 'price', 'cost', 'discount', 'promotion',
        # Общие новости без конкретики
        'новости компании', 'пресс-релиз', 'общая информация', 'о компании',
        'company news', 'press release', 'general info', 'about company',
        # Реклама и маркетинг
        'реклама', 'маркетинг', 'услуги', 'партнерство', 'сотрудничество',
        'advertisement', 'marketing', 'services', 'partnership'
    ]
            ...
            try:
                ...
                if 'items' in search_data:
                    for item in search_data['items']:
                        ...
                        # === УЛУЧШЕНИЕ 5: Улучшенная проверка названия компании с очищенной версией ===
                        company_name_variants = [
                            company_name.lower(),
                            clean_company_name.lower(),  # используем очищенное название
                            company_name.lower().replace('"', '').replace('«', '').replace('»', ''),  # без кавычек
                            clean_company_name.lower().replace('"', '').replace('«', '').replace('»', ''),
                            # Дополнительные варианты без правовых форм
                            re.sub(r'\b(ао|тоо|оао|ип|llp|jsc)\b', '', company_name.lower(), flags=re.IGNORECASE).strip(),
                            re.sub(r'\b(ао|тоо|оао|ип|llp|jsc)\b', '', clean_company_name.lower(), flags=re.IGNORECASE).strip()
                        ]
                        # Убираем пустые варианты
                        company_name_variants = [v for v in company_name_variants if v.strip()]
    ...
    print(f"🔧 [CHARITY_RESEARCH] Применены улучшения: очистка названий, поиск прошлых действий, расширенные ключевые слова")
