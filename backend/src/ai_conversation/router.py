...
async def get_company_charity_info(
    ...
    try:
        ...
        prompt = f"""
        Исходя из приведённых ниже результатов поиска, проанализируй участие компании «{request.company_name}» в благотворительной деятельности:
        ...
        Учитывай посты из соцсетей (Instagram, Facebook и т.д.), если они есть среди ссылок.

        Сформулируй краткий и структурированный ответ по следующим пунктам:
        ...
        4. Основные направления помощи (например, образование, здравоохранение, поддержка детей, культура и т.д.)
        5. Упоминания в социальных сетях (если были обнаружены):
           - Укажи платформу (например, Facebook, Instagram и т.д.)
           - Кратко опиши, о чём был пост, только если информация содержится в ссылке

        ⚠️ Важно: не выдумывай и не домысливай информацию. Используй только факты, указанные в тексте и ссылках. Если данных недостаточно — честно сообщи об этом.

        Если конкретной информации о благотворительности не найдено, выбери один из следующих вариантов:

        - «Информация о благотворительной деятельности компании «{request.company_name}» не найдена в открытых источниках.»
        - «Компания «{request.company_name}» могла участвовать в благотворительности, однако достоверных подтверждений не обнаружено.»
        - «Достоверные сведения о благотворительной активности компании «{request.company_name}» отсутствуют среди найденных материалов.»

        ОБЯЗАТЕЛЬНО заверши ответ блоком "Источники:" со списком всех использованных ссылок в формате:
        
        Источники:
        - [название источника или домен] - [ссылка]
        ...
        Ответь кратко, чётко и на русском языке.
        """
